import { web3Method } from 'actions/web3Redux';
...
function generateWeb3ActionCreator({ networkId, groupName, methodName, dispatch }) {
  ...
  if (!networkApis[networkId] || !networkApis[networkId].web3) {
...
function generateNetworkApi({ networkId, getState, dispatch }) {
  ...
  function gotBlock(block) {
    ...
    dispatch({ type: 'web3Redux/updateNetwork', networkId, payload: { connecting: false, connected: true } });
    ...
    return dispatch({
      type: 'web3Redux/web3MethodSuccess',
      networkId,
      key,
      payload: { value, updated: new Date() }
    });
  }
